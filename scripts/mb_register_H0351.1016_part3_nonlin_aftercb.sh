...
antsRegistration --dimensionality 3 --float 0 --collapse-output-transforms 1 --verbose -n BSpline[5] --write-interval-volumes 100000000 \
  --output [${outputdir}/$(basename ${movingfile})-$(basename ${fixedfile}),${outputdir}/$(basename ${movingfile}).Warped.nii.gz] \
  --use-histogram-matching 0 \
  --initial-moving-transform ${outputdir}/part21Warp.nii.gz \
  --initial-moving-transform ${outputdir}/part20GenericAffine.mat \
